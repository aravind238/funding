...
class PayeesImportNewBankChanges(Command):
    def __init__(self, db=None, filename=None, sheetname="NEW BANK CHANGES"):
        self.db = db
    ...
    def run(self):
        try:
            ...
            return self.import_third_party()
            ...
            file_data = file_data.replace(
                {
                    np.nan: None,
                    0: None,
                    "0": None,  # All fields with 0 is actually NULL
                }
            )

        return file_data

    def import_third_party(self):
        file_data = self.read_file()
        # print("--import new bank changes--", file_data.columns)
        import_payees_list = []
        ...
        for i in file_data.index:
            ...
            # Beneficiary Name
            if (
                "Payee Nickname" in file_data
                and file_data["Payee Nickname"][i]
            ):
                beneficiary_name = str(file_data["Payee Nickname"][i])

            # Bank Account Name
            if (
                "Account Name" in file_data
                and file_data["Account Name"][i]
            ):
                bank_account_name = str(file_data["Account Name"][i])

            # Beneficiary Account Number
            if (
                "Account Number" in file_data
                and file_data["Account Number"][i]
            ):
                beneficiary_account_number = str(file_data["Account Number"][i])

            # Beneficiary Address Line 1
            if (
                "Address" in file_data
                and file_data["Address"][i]
            ):
            ...
            if (
                "Institution" in file_data
                and file_data["Institution"][i]
            ):
                beneficiary_bank_name = str(file_data["Institution"][i])

            # Beneficiary Bank Id
            if (
                "ACH Bank ID" in file_data
                and file_data["ACH Bank ID"][i]
            ):
                ach_bank_id = str(file_data["ACH Bank ID"][i])

            # Beneficiary Bank Id
            if (
                "WIRE Bank ID" in file_data
                and file_data["WIRE Bank ID"][i]
            ):
                wire_bank_id = str(file_data["WIRE Bank ID"][i])

            # Client Code
            if "Client Code" in file_data and file_data["Client Code"][i]:
                lcra_client_accounts_number = str(file_data["Client Code"][i])
            ...
            if client:
                ...
                req_data = {
                    ...
                    "us_wire_banking_info": {
                        "swift_code": None,
                        "institution_routing": wire_bank_id, # Institution Routing (WIRE)
                    },
                    ...
                    "us_ach_banking_info": {
                        ...
                        "bank_name": beneficiary_bank_name,
                        "bank_id_ach": ach_bank_id, # Institution Routing (ACH)
            ...
            print(beneficiary_name, "--import new bank changes - beneficiary--", i)
            print("====end====")
